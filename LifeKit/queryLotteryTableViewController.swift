...
class queryTableViewController: UITableViewController {
        ...
        didSet{
            if let code = lotterycode{
                LotteryHelper.GetMutileLotteryList(code, callback: { (mutilelotterydata) in
                    self.lotteryhistory = mutilelotterydata
                })
            }
        }
    }
    
    private var lotteryhistory:MutileLotteryRootClass?{
        didSet{
    ...
    override func viewDidLoad() {
        ...
        refreshControl?.addTarget(self, action: #selector(queryTableViewController.getData), forControlEvents: .ValueChanged)
        //设置代理和数据源
